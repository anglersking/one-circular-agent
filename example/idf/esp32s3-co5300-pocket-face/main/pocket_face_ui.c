...
#define POCKET_SOURCE_WIDTH 172
#define POCKET_SOURCE_HEIGHT 320
#define POCKET_SCALE 373
...
#define POCKET_ASSET(start_symbol, end_symbol, asset_name)                \
    {                                                                    \
        .image = {                                                       \
            .header = {                                                 \
                .magic = LV_IMAGE_HEADER_MAGIC,                         \
                .cf = LV_COLOR_FORMAT_RAW,                              \
                .w = POCKET_SOURCE_WIDTH,                               \
                .h = POCKET_SOURCE_HEIGHT,                              \
            },                                                          \
            .data = start_symbol,                                       \
        },                                                              \
        .end = end_symbol,                                              \
...
static pocket_asset_t s_faces[] = {
    POCKET_ASSET(_binary_twece_gif_start, _binary_twece_gif_end, "twece"),
    POCKET_ASSET(_binary_anger_gif_start, _binary_anger_gif_end, "anger"),
    POCKET_ASSET(_binary_disdain_gif_start, _binary_disdain_gif_end, "disdain"),
    POCKET_ASSET(_binary_excited_gif_start, _binary_excited_gif_end, "excited"),
    POCKET_ASSET(_binary_once_gif_start, _binary_once_gif_end, "once"),
};

static pocket_asset_t s_charge =
    POCKET_ASSET(_binary_charge_gif_start, _binary_charge_gif_end, "charge");
...
static void load_asset(pocket_asset_t *asset, int32_t loop_count)
{
    ...
    // LVGL uses 256 as 1.0x. 373 scales 172x320 to about 251x466 while
    // preserving the original portrait aspect ratio on the round display.
    lv_image_set_scale(s_gif, POCKET_SCALE);
    lv_obj_center(s_gif);
...
void pocket_face_ui_init(lv_display_t *display, esp_lcd_panel_handle_t panel)
{
    ...
    ESP_LOGI(TAG, "tap: next face; hold: charge mode; touch wakes after dim");
}
