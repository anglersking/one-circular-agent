...
static const char *TAG = "POCKET_EYES_FAST";
...
void app_main(void)
{
    ESP_LOGI(TAG, "Starting isolated Pocket fast-eyes demo");
    ...
    // CO5300 QSPI needs an internal DMA-capable line buffer. Keeping the LVGL
    // display buffer out of PSRAM prevents the SPI bounce-buffer allocation
    // failure seen with large GIF assets.
    ...
    if (esp_lv_adapter_lock(portMAX_DELAY) == ESP_OK) {
        pocket_face_ui_init(display, panel);
        esp_lv_adapter_unlock();
    }

    ESP_LOGI(TAG, "Pocket fast-eyes ready: tap next, hold charge, 10s dim");
}
