...
[`example/idf/`](example/idf/README.md) 是 ESP-IDF 例程索引。原厂硬件基准工程是 [`esp32s3-co5300-osptek-lvgl9`](example/idf/esp32s3-co5300-osptek-lvgl9/README.md)，用于验证 CO5300 显示和 CST820 触摸；[`esp32s3-co5300-deepseek-quota`](example/idf/esp32s3-co5300-deepseek-quota/README.md) 单独提供像素 Agent、Wi-Fi、DeepSeek 余额和柱状历史；[`esp32s3-co5300-grok-agent`](example/idf/esp32s3-co5300-grok-agent/README.md) 只提供纯本地 LVGL blob 表情和触摸交互，不包含网络、余额或外部受限素材；ONE 的 UI 工程是 [`esp32s3-co5300-agent-watch`](example/idf/esp32s3-co5300-agent-watch/README.md)，运行来自 SquareLine Studio 的 LVGL 9 圆形界面。
